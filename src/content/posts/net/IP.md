...
| 特性 | 路由表(RIB, Routing Information Base) | 转发表(FIB, Forwarding Information Base) |
| :--- | :--- | :--- |
| **所属平面** | **控制平面** (Control Plane) | **数据平面** (Data Plane) |
| **存储介质** | 控制处理器内存 (CPU/RAM) | 接口线卡专用硬件 (**TCAM**/SRAM) |
| **内容** | 包含**所有**候选路由、管理距离、度量值 | 只包含**最优**路由的转发必备信息 |
| **更新频率** | 由路由协议动态触发更新 | 由RIB定期或触发式同步下发 |
| **查找方式** | 软件匹配，速度相对较慢 | **硬件级并行匹配**，支持线速(Line Rate)转发 |
| **作用** | 路由决策，从多条可选路径中选出最优路由 | 高速转发查找 |
...
- **硬件级速度**：高性能路由器利用 **TCAM (三态内容寻址存储器)**，在一个时钟周期内完成所有条目的并行比对。
- **逐包处理**：每个到达的 IP 包独立决策，不考虑全局拓扑

在 `IP层`，数据平面的具体行为就是**转发表查找 + 出口选择 + MAC 封装 + 发送**：
- 读取 IP 首部目标地址，并验证首部校验和(IPv4)
- TTL 减 1，若为 0 则丢弃并回送 ICMP 超时。
- 在 转发表FIB 中执行**最长前缀匹配**(LPM, Longest Prefix Match)，确定出口网卡和下一跳 IP
- 由于 TTL等 改变，利用增量算法更新 IP 首部校验和。
- 通过 ARP缓存 获取下一跳 MAC，封装以太网帧，交给网卡DMA发送
...
#### 传统分布式控制平面（路由表/RIP/OSPF/BGP）
路由器之间通过路由协议交换链路状态或路径信息，各自独立计算路由，填充本地路由表。不同协议的选路逻辑有所区别：
- **`IGP`（RIP、OSPF等内部网关协议）**：交换链路状态或距离向量，**各自运行最短路径算法**（RIP 使用 Bellman-Ford、OSPF 使用 Dijkstra），计算到达各目标网络的最短路径。
- **`EGP`（BGP等外部网关协议）**：是**路径矢量**协议，不运行最短路径算法；各路由器交换 AS 路径和可达性信息，基于 Local Preference、AS Path 长度、MED 等**多维策略属性**进行选路，服从网络策略而非单纯最短路径。

```text
路由协议报文(RIP Response / OSPF LSA / BGP UPDATE)
         ↓
IGP：各路由器独立运行最短路径算法（Bellman-Ford / Dijkstra）
EGP：各路由器依据策略属性（AS Path、Local Pref 等）执行策略选路
         ↓
选出最优路由，填充本地路由表（RIB）
         ↓
...
1. 读取 IP 首部的目标地址
2. 遍历转发表，找到与目标地址网络号匹配的记录。若有多条匹配，选择**最长前缀匹配**(最具体的路由)
3. 按照转发表记录将包转发给下一跳路由器
### 环回地址
`127.0.0.1`(`localhost`)是环回地址，用于**同一主机上的进程间网络通信**。发往该地址的报文不经过网卡，直接在内核协议栈内部处理。
- **环回地址范围**：`127.0.0.0/8` (127.0.0.0 ~ 127.255.255.255)
通常使用 `127.0.0.1`。整个 127.x.x.x 范围都指向本地，可用于多服务隔离（如 `127.0.0.1` 给 Web 服务，`127.0.0.2` 给数据库服务）

---

## 四、IP 分片与重组：一个过时的“补丁”
![IP首部](images/IP首部.png)
