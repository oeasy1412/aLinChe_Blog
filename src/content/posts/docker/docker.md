...
## Q & A:
- Q1: 为什么我不能通过docker隔离不同版本的驱动程序
  - A1: 驱动通过硬件描述信息初始化之后就是一个内存中的程序，感觉可以做隔离，但别忘了内核是共享的，这个内存其实是内核空间中的内存，常见的驱动是一种​​内核模块​​。docker 本质是 **​​共享单一宿主机内核**​ ​的进程隔离环境。设备驱动是暴露read()write()ioctl()等接口供容器使用的，容器就一个用户空间的沙盒，没有驱动这一说。所以哪怕是--privileged，insmod也是到宿主机上，容器什么都没有