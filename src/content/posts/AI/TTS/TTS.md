...
- **核心思想**：「**非自回归(Non-Autoregressive, NAR) + 时长预测器(`Duration Predictor`)**」。彻底抛弃 Decoder 的自回归，改为：一个 Transformer Encoder 把音素编成隐状态，再由一个**时长预测器** 预测每个音素应该撑几帧，然后把每个音素的隐状态**按预测时长复制**铺开成 mel 帧，最后一个 Transformer 映射成 mel。
- **突破**：
